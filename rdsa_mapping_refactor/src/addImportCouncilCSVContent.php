...
class addImportCouncilCSVContent {

  /**
   * @throws \Drupal\Core\Entity\EntityStorageException
   */
  public static function addImportCouncilCSVContentItem($item, &$context): void {
    $context['sandbox']['current_item'] = $item;
    $message = 'Creating ' . $item['Council'];
    $results = array();
  ...
  function addImportIndustryCSVContentItemCallback($success, $results, $operations): void {
    ...
    \Drupal::logger('rdasa_council_import')->error("Councils csv imported");
    \Drupal::logger('rdasa_council_import')->error($message);
...
function create_taxonomy($item): void {
  $term_data['vid'] = 'council_areas';
  $term_data['name'] = $item["Council"];
